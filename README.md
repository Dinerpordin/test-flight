...
> **Status: LIVE** — Running in Duffel live mode. Real bookings are processed via the Duffel Orders API.
...
- Full booking flow via Duffel Orders API ✅
...
test-flight/
  ...
  app/
    ...
    api/
      ...
      book/
        route.ts        # POST /api/book (Duffel Orders API)
  components/
    Navbar.tsx
    SearchForm.tsx
    ResultsList.tsx
...
|---|---|---|
| `DUFFEL_ACCESS_TOKEN` | Yes | Duffel API live token (`duffel_live_...`) |
...
Get your live Duffel token at [app.duffel.com](https://app.duffel.com/) → Access tokens → **Live** tab.
...
Creates a real Duffel order. Requires `offerId` and `passenger` details.
...
Ensure `DUFFEL_ACCESS_TOKEN` is set to your `duffel_live_...` token in Vercel for the production environment.
